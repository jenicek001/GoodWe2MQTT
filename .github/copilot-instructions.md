...
- **Service Pattern**: Cada inverter defined via indexed `G2M_GOODWE_INVERTERS_*` variables is managed by an instance of `Goodwe_MQTT` class.
...
- [.env](.env): Central configuration for MQTT brokers, inverter IPs/serials, and logging.
...
- **Configuration**: Always update `.env` / `.env.example` for deployment settings.
